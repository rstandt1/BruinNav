...
int main()
{
	string line = "34.0555267,       -118.4796954 34.0555356,-118.4798135";
	cout << line.substr(0, line.find(",")) << endl;
	cout << line.substr(line.find_first_not_of(", ", line.find(",")), line.find(" ", line.find_first_not_of(", ", line.find(","))) - line.find_first_not_of(", ", line.find(",")));
	cout << endl;
	cout << line.substr(line.find_first_not_of(" ", line.find(" ", line.find_first_not_of(", ", line.find(",")))),
	...
	cout << endl;
	cout << line.substr(line.find_first_not_of(", ", line.find(",", line.find_first_not_of(", ", line.find(",")))), line.length()) << endl;*/
	...
	am.init(ml);
	GeoCoord spot;
	if (am.getGeoCoord("DIDDY RIESE", spot))
		cout << "YAY" << endl;
	SegmentMapper sm;
	sm.init(ml);
	if (!sm.getSegments(spot).empty())
	{
		for (size_t i = 0; i < sm.getSegments(spot).size(); i++)
			cout << sm.getSegments(spot)[i].streetName << endl;
	}
	cout << endl << endl << endl;
	Navigator z;
	z.loadMapData("mapdata.txt");
	vector<NavSegment> order;
	z.navigate("diddy riese", "Whole Foods Market", order);
